...
namespace OpenCVSharp
{
    public class FormCh03 : Form
    {
        ...
        private readonly GroupBox grpCannyParams = new GroupBox();
        private readonly TrackBar trackCannyThresh1 = new TrackBar();
        private readonly TrackBar trackCannyThresh2 = new TrackBar();
        private readonly Label lblCannyThresh1 = new Label();
        private readonly Label lblCannyThresh2 = new Label();

        private readonly PictureBox pbLatency = new PictureBox(); // Latency Breakdown Chart
        ...
        // Latency tracking metrics (ms)
        private double lastGrabTime = 0;
        private double lastFilterTime = 0;
        private double lastFormatTime = 0;
        ...
        private void InitializeTheoryTab()
        {
            ...
            string theoryText = "■ [카메라 불러오기 및 설정 (CvCapture)]\r\n" +
        ...
        private void InitializeLabTab()
        {
            ...
            var panel = new Panel
            {
                ...
                Size = new Size(310, 680), // Increased height to fit new controls
                BorderStyle = BorderStyle.FixedSingle,
            ...
            // Canny parameters GroupBox
            grpCannyParams.Text = "⚙ Canny 에지 임계값 조절 (Threshold)";
            grpCannyParams.Location = new Point(15, y);
            grpCannyParams.Size = new Size(270, 75);
            grpCannyParams.Font = new Font("Malgun Gothic", 8F, FontStyle.Bold);
            grpCannyParams.Enabled = false;

            lblCannyThresh1.Text = "Thresh1: 50";
            lblCannyThresh1.Location = new Point(10, 20);
            lblCannyThresh1.Size = new Size(80, 15);
            grpCannyParams.Controls.Add(lblCannyThresh1);

            trackCannyThresh1.Minimum = 0;
            trackCannyThresh1.Maximum = 255;
            trackCannyThresh1.Value = 50;
            trackCannyThresh1.Location = new Point(90, 15);
            trackCannyThresh1.Size = new Size(170, 25);
            trackCannyThresh1.TickStyle = TickStyle.None;
            trackCannyThresh1.Scroll += (s, e) => lblCannyThresh1.Text = $"Thresh1: {trackCannyThresh1.Value}";
            grpCannyParams.Controls.Add(trackCannyThresh1);

            lblCannyThresh2.Text = "Thresh2: 150";
            lblCannyThresh2.Location = new Point(10, 45);
            lblCannyThresh2.Size = new Size(80, 15);
            grpCannyParams.Controls.Add(lblCannyThresh2);

            trackCannyThresh2.Minimum = 0;
            trackCannyThresh2.Maximum = 255;
            trackCannyThresh2.Value = 150;
            trackCannyThresh2.Location = new Point(90, 40);
            trackCannyThresh2.Size = new Size(170, 25);
            trackCannyThresh2.TickStyle = TickStyle.None;
            trackCannyThresh2.Scroll += (s, e) => lblCannyThresh2.Text = $"Thresh2: {trackCannyThresh2.Value}";
            grpCannyParams.Controls.Add(trackCannyThresh2);

            panel.Controls.Add(grpCannyParams);
            y += 82;
            ...
            // Real-Time Latency Breakdown Chart
            addHeader("📊 실시간 지연 시간 분석 (Latency Chart)");
            pbLatency.Location = new Point(15, y);
            pbLatency.Size = new Size(270, 60);
            pbLatency.BorderStyle = BorderStyle.FixedSingle;
            pbLatency.BackColor = Color.FromArgb(245, 245, 245);
            pbLatency.Paint += PbLatency_Paint;
            panel.Controls.Add(pbLatency);
            y += 68;

            lblResolutionInfo.Location = new Point(15, y);
            lblResolutionInfo.Size = new Size(275, 18);
            ...
            lblMeasuredFps.Size = new Size(275, 18);
            ...
            lblStatus.Size = new Size(275, 18);
            ...
            string theorySummaryText = "【핵심 이론 및 원리】\r\n" +
                                       ...
                                       "   - 하드웨어로부터 실시간 입력을 얻습니다.\r\n" +
                                       "2. **지연 성능 (Latency)**\r\n" +
                                       "   - **Grab**: 하드웨어에서 프레임을 퍼올리는 시간\r\n" +
                                       "   - **Filter**: OpenCV 알고리즘 처리 시간\r\n" +
                                       "   - **Convert**: C++ 포인터를 C# Bitmap으로 복사하는 마샬링 시간\r\n" +
                                       "   - **Render**: 화면에 갱신하는 드로잉 시간\r\n" +
                                       "3. **리소스 반납**: `Dispose()` 필수!\r\n" +
                                       "   - 미반납 시 카메라 장치 락이 걸려 다른 앱에서 쓸 수 없습니다.";
            RichTextHelper.SetMarkdown(txtTheory, theorySummaryText);

            tabLab.Controls.Add(new TextBox
            {
                Text = "실습 가이드: PC에 연결된 웹캠 번호를 입력한 뒤 [장치 켜기]를 누르세요. 필터에 따른 실시간 연산 지연율(Latency Chart)의 등락을 분석하세요.",
        ...
        {
            grpCannyParams.Enabled = (cmbFilter.SelectedIndex == 2);
        }

        private void PbLatency_Paint(object sender, PaintEventArgs e)
        {
            double total = lastGrabTime + lastFilterTime + lastFormatTime + lastRenderTime;
            if (total <= 0) total = 1.0;

            int w = pbLatency.Width;
            int h = pbLatency.Height;

            // Draw stacked bar
            float x1 = 0;
            float w1 = (float)(lastGrabTime / total * w);
            float w2 = (float)(lastFilterTime / total * w);
            float w3 = (float)(lastFormatTime / total * w);
            float w4 = (float)(lastRenderTime / total * w);

            // Stacked block representation
            e.Graphics.FillRectangle(Brushes.DodgerBlue, x1, 0, w1, 18);
            e.Graphics.FillRectangle(Brushes.Orange, x1 + w1, 0, w2, 18);
            e.Graphics.FillRectangle(Brushes.ForestGreen, x1 + w1 + w2, 0, w3, 18);
            e.Graphics.FillRectangle(Brushes.Crimson, x1 + w1 + w2 + w3, 0, w4, 18);

            e.Graphics.DrawRectangle(Pens.LightGray, 0, 0, w - 1, 18);

            // Draw text values (details)
            using (var font = new Font("Consolas", 7.5F, FontStyle.Bold))
            {
                string text = $"Grab: {lastGrabTime:F1}ms | Filter: {lastFilterTime:F1}ms\nConv: {lastFormatTime:F1}ms | Render: {lastRenderTime:F1}ms";
                e.Graphics.DrawString(text, font, Brushes.Black, 5, 23);
            }
        }
        ...
        private void RenderTimer_Tick(object sender, EventArgs e)
        {
            ...
            frame = capture.QueryFrame();
            sw.Stop();
            lastGrabTime = sw.Elapsed.TotalMilliseconds;
            sw.Reset();
                ...
                switch (cmbFilter.SelectedIndex)
                {
                    case 1: // Grayscale
                        using (var gray = new Mat())
                        {
                            ...
                            sw.Stop();
                            lastFilterTime = sw.Elapsed.TotalMilliseconds;
                            sw.Reset();

                            sw.Start();
                            processedBitmap = BitmapConverter.ToBitmap(gray);
                            sw.Stop();
                            lastFormatTime = sw.Elapsed.TotalMilliseconds;
                        }
                        break;
                    case 2: // Canny Edge
                        using (var gray = new Mat())
                        ...
                        {
                            Cv2.CvtColor(mat, gray, ColorConversion.BgrToGray);
                            double t1 = (double)trackCannyThresh1.Value;
                            double t2 = (double)trackCannyThresh2.Value;
                            Cv2.Canny(gray, edge, t1, t2);
                            sw.Stop();
                            lastFilterTime = sw.Elapsed.TotalMilliseconds;
                            sw.Reset();
